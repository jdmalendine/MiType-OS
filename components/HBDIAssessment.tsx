        ...
        <Card className="max-w-4xl mx-auto p-8 md:p-12">
            <div className="flex items-center mb-8">
                ...
                <div className="flex-grow text-center">
                    <h2 className="text-3xl font-black mb-4 font-header tracking-tighter italic uppercase">M-CET STEP 2: HBDI ASSESSMENT</h2>
                </div>
            </div>
            <div className="text-center mb-12">
                <p className="text-brand-text-muted font-medium leading-relaxed max-w-2xl mx-auto">For each question, choose the option that best describes your cognitive preference. Precision is key for neural calibration.</p>
            </div>
                ...
                {hbdiQuestions.map((item, i) => (
                    <div key={i} className="border-b border-brand-border/20 pb-10 last:border-0">
                        <div className="flex items-start gap-4 mb-6">
                            ...
                            <p className="text-xl font-bold font-header tracking-tight leading-snug">{item.q}</p>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                           {(['a', 'b', 'c', 'd'] as const).map(opt => (
                                <label key={opt} className={`flex items-center space-x-4 p-5 rounded-2xl border-2 transition-all duration-300 cursor-pointer group ${answers[i] === opt ? selectedOptionStyles[opt] : `bg-white/5 ${optionStyles[opt]}`}`}>
                                    ...
                                    <span className="font-bold tracking-wide">{ (item as any)[opt] }</span>
                                </label>
                           ))}
                        </div>
                    </div>
            ...
            <Button onClick={handleSubmit} className="mt-10 w-full py-6 text-xl font-black tracking-widest uppercase shadow-[0_0_30px_rgba(99,102,241,0.2)]">Continue Neural Mapping</Button>
        </Card>
    );
};
